...
(function () {
	...
	Chart.prototype._updateOptions = function () {
		...
		if (this._options.zoomEnabled) {
            console.log("DuyDN1: here you are, zoomEnabled =============> !!!!!!!!!!!!!")
			if (!this._zoomButton) {
				console.log("go to != zoombutton ")
			...
			if (!this._resetButton) {
								console.log("go to != _resetButton ")
				...
				addEvent(this._resetButton, "click", function () {
					...
					//DuyDN1 update startpoint and EndPoint
					returnStartPoint = {x:0, y: 0};
					returnEndPoint = {x:0, y: 0};
			...
			if (!this.zoomEnabled && !this.panEnabled) {
				if (!this._zoomButton) {
					...
					console.log("show(_this._zoomButton, _this._resetButton); ")
	...
	Chart.prototype._plotAreaMouseDown = function (x, y) {
		...
		console.log(" _plotAreaMouseDown is called")
		this.dragStartPoint = {
			x: x, y: y
		};
	}

	var returnStartPoint = {
		x: 0, y: 0
	}
	var returnEndPoint = {
		x: 0, y: 0
	}
	Chart.prototype._getStartPoint = function(){
		console.log(" _getStartPoint is called ")
		console.log(returnStartPoint)
		return returnStartPoint;
	}
	Chart.prototype._getEndPoint = function(){
		console.log(" _getEndPoint is called")
	...
	Chart.prototype._calculatorPoints = function(){
		...
		{
			returnStartPoint.x = parseInt((this.dragStartPoint.x - this.plotArea.x1)*Delta2/Delta + returnStartPoint.x)
			returnStartPoint.y = this.dragStartPoint.y
		}
		if(returnEndPoint.x == 0)
		{
			returnEndPoint = this.dragEndPoint
		}
		else
		{
			returnEndPoint.x = parseInt(returnEndPoint.x - (this.plotArea.x2 - this.dragEndPoint.x)*Delta2/Delta)
			returnEndPoint.y = this.dragEndPoint.y
		}
	}
			...
			if (this.isDrag) {
				...
				if (this.panEnabled || this.zoomEnabled) {
					if (this.panEnabled) {
						...
						for (var i = 0; i < this._axes.length; i++) {
							...
							if (axis.viewportMinimum < axis.minimum) {
								...
								console.log(" axis.viewportMinimum < axis.minimum ")
					...
					if (reRender) {
						...
						//DuyDN1: calculator the start point and end point
						this._calculatorPoints()
		...
		if (this.isDrag && this.plotInfo.axisPlacement !== "none") {
			...
			if (((!zoomPX || Math.abs(dragDeltaPX) > 2) || (!zoomPY || Math.abs(dragDeltaPY) > 2)) && (this.panEnabled || this.zoomEnabled)) {
				if (this.panEnabled) {
					...
					console.log(" this.dragStartPoint.x = " + this.dragStartPoint.x + " this.dragStartPoint.y= " +this.dragStartPoint.y  )
					var rect = {
						x1: zoomPX ? this.dragStartPoint.x : this.plotArea.x1,
						y1: zoomPY ? this.dragStartPoint.y : this.plotArea.y1,
...
//DuyDN1: handle mouse event
	EventManager.prototype.mouseEventHandler = function (ev) {
		...
		if (id && typeof (this.objectMap[id]) !== "undefined") {
			console.log("DuyDN1:id = this.chart.getObjectAtXY(xy.x, xy.y, false); line=15593 ")
	...
	var CanvasJS = {
		...
		Chart: function (containerId, options) {
			...
			//DuyDN1: add more API
			this.plotAreaMouseDown = function (x,y) {
				_chart._plotAreaMouseDown(x,y)
			};
			this.plotAreaMouseUp = function (x,y) {
				_chart._plotAreaMouseUp(x,y)
			};
			this.getStartPoint = function() {
				return _chart._getStartPoint();
			}
			this.getEndPoint = function() {
				return _chart._getEndPoint();
			}
			//DuyDN1: end
			//console.log(_chart);
